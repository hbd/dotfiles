...
(use-package go-mode
  :ensure t
  :init
  (use-package go-rename ;; Ensure go-rename is available.
    :ensure t)
  ...
  :bind (("M-*" .     pop-tag-mark)     ;; Pop mark after jumping to definition.
	 ("C-c C-i" . go-goto-imports)  ;; Jump to import list.
	 ("C-c C-j" . godef-jump)       ;; Jump to import list.
	 ("C-c C-e" . go-rename)        ;; Call go-rename at point.
	 ("C-c d" .   godoc-at-point)   ;; Open godoc from point in new pane.
	 ("C-c C-c" . comment-region)   ;; Comment region.
	 ("C-c C-u" . uncomment-region) ;; Uncomment region.
  ...
  :config (setq gofmt-command "goimports") ;; Use goimports instead of gofmt.

  :hook (before-save . gofmt-before-save) ;; Import before saving.
  )
...
;;             (global-set-key (kbd "C-c C-j") 'godef-jump)       ;; Jump to import list.
